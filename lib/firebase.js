...
// 앱 초기화 (Singleton Pattern for Next.js HMR)
import { getApps, getApp } from 'firebase/app';
import { getFirestore, initializeFirestore, persistentLocalCache, persistentMultipleTabManager, doc, getDoc, setDoc } from 'firebase/firestore';
...
// Messaging 초기화 (SSR 방지 및 지원 여부 확인)
export const messaging = typeof window !== 'undefined'
  ? isSupported().then(yes => yes ? getMessaging(app) : null).catch(() => null)
  : Promise.resolve(null);

// Analytics 초기화 (SSR 방지)
export const analyticsPromise = typeof window !== 'undefined'
  ? analyticsIsSupported().then(yes => yes ? getAnalytics(app) : null).catch(() => null)
  : Promise.resolve(null);
...
export async function loginWithKakao() {
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      if (!plugin) {
        throw new Error('카카오 플러그인을 찾을 수 없습니다.');
      }
    ...
    const { data } = await verifyKakaoToken({ accessToken }).catch((error) => {
      // email-conflict 에러를 그대로 위로 던지기
    ...
    if (data.customToken) {
      ...
      console.log('✅ 로그인 유저 이메일:', user.email);
      return user;
    } else {
    ...
    // email-conflict는 alert 없이 위로만 던지기
    if (error.message === 'email-conflict') {
      throw error;
    }
