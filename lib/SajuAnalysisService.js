...
class SajuAnalysisService {
  ...
  async analyze(config) {
    ...
    if (customValidation && !(await customValidation(params, this))) { // ⭐️ await 추가
      return null;
    }
    ...
    try {
      ...
      const currentCredits = this.userData?.credits || 0;
      let useCredit = this.maxEditCount <= editCount;
      ...
      if (useBackground && this.user) {
        ...
        try {
          ...
          const bgParams = { ...params, useCredit: !!useCredit };
        ...
        try {
          ...
          if (isGuestMode && guestId && guestCollection) {
            ...
            if (useCredit) {
              ...
              console.log('✅credits', saveData.crcedits, saveData);
              //여기로 다시
...
class AnalysisPresets {
    ...
    return {
      ...
      customValidation: async (p, service) => {
        ...
        if (!service.userData?.birthDate) {
          alert(service.uiText?.saveFirst?.[this.language] || '생년월일을 먼저 저장해주세요.');
          return false;
        }
        return true;
      },
  ...
  static saza(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => ({
        ...
        '{{target_FORMAT}}': prompts['prompt/saza_format'],
        ...
        '{{langPrompt}}': service.langPrompt?.(this.language) || '없음',
        '{{hanjaPrompt}}': service.hanja?.(this.language) || '',
      }),
  ...
  static sazaGuest(params, guestId) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        const displayName =
          service.userData?.displayName || (this.language === 'ko' ? '의뢰자' : 'guest');
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          ...
          '{{todayInfo}}': `현재 시각:${new Date().toLocaleString()}. 2026년=병오년. `,
          '{{langPrompt}}': service.langPrompt?.(this.language) || '',
          '{{hanjaPrompt}}': service.hanja?.(this.language) || '',
        };
      },
  ...
  static match(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          '{{matchFormat}}': prompts['prompt/match_format'],
  ...
  static newYear(params) {
    ...
    return {
      ...
      buildPromptVars: (prompts, p, service) => ({
        '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
        '{{targetFormat}}': prompts[`prompt/new_year_format_${p.language}`],
        '{{gender}}': p.gender,
  ...
  static daily(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          '{{targetFormat}}': prompts[`prompt/daily_format_${p.language}`],
          '{{gender}}': p.gender,
          '{{mySaju}}': service.getSajuString(p.saju),
  ...
  static dailySpecific(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          '{{targetFormat}}': prompts[`prompt/daily_s_${p.language}`],
          '{{gender}}': p.gender,
          '{{mySaju}}': service.getSajuString(p.saju),
          '{{todayDate}}': today,
          '{{todaySajuText}}': todaySajuText,
    ...
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        const birthMethodLabel = p.birthMethod === 'natural'
          ? (this.language === 'ko' ? '자연분만' : 'Natural Birth')
          : (this.language === 'ko' ? '제왕절개' : 'Cesarean Section');
        ...
        return {
          ...
          '{{userGender}}': p.gender === 'female' ? (this.language === 'ko' ? '여성' : 'Female') : (this.language === 'ko' ? '남성' : 'Male'),
          '{{partnerGender}}': p.gender === 'female' ? (this.language === 'ko' ? '남성' : 'Male') : (this.language === 'ko' ? '여성' : 'Female'),
          '{{startDate}}': p.startDate,
          '{{endDate}}': p.endDate,
          '{{purpose}}': p.purpose,
          '{{birthMethod}}': birthMethodLabel,
          '{{babyGender}}': p.babyGender === 'boy' ? (this.language === 'ko' ? '남아' : 'Boy') : p.babyGender === 'girl' ? (this.language === 'ko' ? '여아' : 'Girl') : (this.language === 'ko' ? '성별모름' : 'Unknown'),
