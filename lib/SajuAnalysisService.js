    ...
    try {
      ...
      if (useBackground && this.user) {
        ...
        try {
          ...
          const bgParams = { ...params, useCredit: !!useCredit, language: this.language };
  ...
  static basic(params, sajuData) {
    return {
      ...
      customPromptBuilder: async (p, service) => {
        return await createPromptForGemini(sajuData, service.language);
      },
        ...
        return {
          ...
          usageHistory: {
            ZApiAnalysis: {
              result,
              saju: p.saju,
              language: service.language || this.language || 'ko', // ✅ 다중 fallback
              gender: p.gender,
  ...
  static daily(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          '{{targetFormat}}': prompts[`prompt/daily_format_${service.language}`],
          '{{gender}}': p.gender,
          '{{mySaju}}': service.getSajuString(p.saju),
  ...
  static dailySpecific(params) {
    return {
      ...
      buildPromptVars: (prompts, p, service) => {
        ...
        return {
          '{{DEFAULT_INSTRUCTION}}': prompts['prompt/default_instruction'],
          '{{targetFormat}}': prompts[formatKey] || '', // service.language 대신 위에서 정한 lang 사용
          '{{gender}}': p.gender,
          '{{mySaju}}': service.getSajuString(p.saju),
          '{{todayDate}}': p.selectedDate || new Date(),
          '{{todaySajuText}}': p.sajuDate ? `${p.sajuDate.sky3}${p.sajuDate.grd3}년...` : '', // 위에서 만든 변수 적용
          '{{displayName}}': service.getDisplayName(),
          '{{question}}': p.question || '',
          '{{langPrompt}}': service.langPrompt?.(lang) || '',
