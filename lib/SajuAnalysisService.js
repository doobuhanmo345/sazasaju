...
class SajuAnalysisService {
  ...
  checkUsageLimit() {
    if (!this.userData) {
      return true; // userData 없으면 일단 통과 (로그인 체크는 analyze에서)
    }

    const currentCount = this.userData.editCount || 0;
    const currentCredits = this.userData.credits || 0;
    const freeRemaining = Math.max(this.maxEditCount - currentCount);
    const totalRemaining = freeRemaining + currentCredits;

    console.log('this.maxeditcount', this.maxEditCount, 'user.editcount', currentCount, 'currentCredits', currentCredits, `[checkUsageLimit] Free: ${freeRemaining}, Credits: ${currentCredits}, Total: ${totalRemaining}`);

    if (totalRemaining <= 0) {
      console.log('❌ No usage left');
    ...
    try {
      ...
      if (!skipUsageCheck && !isGuestMode) {
        ...
        if (currentCount >= this.maxEditCount) {
          ...
          console.log(`🆓 프리티어 (${currentCount}/${this.maxEditCount})`);
        }
      }
