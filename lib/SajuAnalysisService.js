...
class SajuAnalysisService {
   ...
   * - onSnapshot으로 결과 대기
   */
  async analyzeInBackground(prompt, userId, targetPath = null, analysisTitle = null) {
  ...
  async analyze(config) {
    ...
    try {
      ...
      const analysisTitle = getAnalysisTitle(type, this.language, params);
      ...
      if (useBackground && this.user) {
        ...
        try {
          result = await this.analyzeInBackground(fullPrompt, this.user.uid, targetPath, analysisTitle);
        } catch (bgError) {
