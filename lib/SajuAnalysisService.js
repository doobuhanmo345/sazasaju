...
const getResultPath = (type, currentPath, params) => {
  ...
  const pathMap = {
    ...
    'match': '/saju/match', // Match results are shown inline, so go to the main page
    'selbirth': '/saju/selbirth/result',
    'seldate': '/saju/seldate/result',
  };

  if (type === 'dailySpecific' && params?.type) {
    console.log('[getResultPath] dailySpecific routing - params.type:', params.type);
    if (params.type === 'firstdate') return '/saju/date/result';
    if (params.type === 'interview') return '/saju/interview/result';
  }

  const resultPath = pathMap[type] || currentPath;
  console.log('[getResultPath] type:', type, 'resultPath:', resultPath);
  return resultPath;
};
...
class SajuAnalysisService {
   ...
   * - onSnapshot으로 결과 대기
   */
  async analyzeInBackground(prompt, userId) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        timeoutId = setTimeout(() => {
          ...
          reject(new Error('Background analysis timeout (5 minutes)'));
        }, TIMEOUT_MS);

        // 3. 실시간 리스너 설정
        unsubscribe = onSnapshot(doc(db, 'analysis_queue', queueRef.id), (snapshot) => {
          ...
          if (data.status === 'completed') {
            ...
            resolve(data.airesult);
          } else if (data.status === 'failed') {
            clearTimeout(timeoutId);
            unsubscribe();
            console.error('[analyzeInBackground] Analysis failed:', data.error);
  ...
  async analyze(config) {
    ...
    if (this.user) {
      try {
        await firestore.updateDoc(firestore.doc(db, 'users', this.user.uid), { isAnalyzing: true });
      } catch (e) { console.error('Failed to set lock:', e); }
    }
    ...
    try {
      ...
      const useBackground = config.forceBackground ||
        (typeof document !== 'undefined' && document.visibilityState !== 'visible');

      if (useBackground && this.user) {
        console.log('🔄 Using background analysis (app not visible or forced)');
        try {
          result = await this.analyzeInBackground(fullPrompt, this.user.uid);
        } catch (bgError) {
          console.warn('⚠️ Background analysis failed, falling back to direct call:', bgError);
          result = await fetchGeminiAnalysis(fullPrompt);
        }
      } else {
        console.log('⚡ Using direct analysis (app visible)');
        result = await fetchGeminiAnalysis(fullPrompt);
      }
        ...
        if (this.user) {
          try {
            const notifMsg = this.language === 'ko'
              ? `${this.uiText?.analysisComplete?.ko || '사주 분석이 완료되었습니다.'}`
            ...
            await addDoc(collection(db, 'notifications'), {
              ...
              createdAt: serverTimestamp(),
              createdAt: serverTimestamp(),
              targetPath: getResultPath(type, window.location.pathname, params)
            });
          } catch (notifErr) {
      ...
      if (this.user) {
        try {
          await firestore.updateDoc(firestore.doc(db, 'users', this.user.uid), { isAnalyzing: false });
