...
class SajuAnalysisService {
  ...
  async analyze(config) {
    ...
    if (this.user) {
      ...
      try {
        await firestore.setDoc(firestore.doc(db, 'users', this.user.uid), { isAnalyzing: true }, { merge: true });
    ...
    try {
          ...
          try {
            const labels = {
              ...
              en: {
                ...
                interview: 'Interview Saju Analysis',
                firstdate: 'First Date Analysis'
              }
            };
      ...
      if (this.user) {
        try {
          await firestore.setDoc(firestore.doc(db, 'users', this.user.uid), { isAnalyzing: false }, { merge: true });
