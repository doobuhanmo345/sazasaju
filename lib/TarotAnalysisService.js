...
class TarotAnalysisService {
  ...
  // [NEW] Usage Limit Check Helper
  checkUsageLimit = async (cost = 1) => {
    if (!this.userData) {
      return true; // Allow if no user data (login check is separate)
    }

    const currentCount = this.userData.editCount || 0;
    const currentCredits = this.userData.credits || 0;
    const freeRemaining = Math.max(0, this.maxEditCount - currentCount);
    const totalRemaining = freeRemaining + currentCredits;

    if (this.handleCancelHelper && totalRemaining < cost) {
      this.handleCancelHelper();
      console.log('🚫 No usage/credits left. Redirecting...');
      ...
      if (confirm(msg)) {
        console.log('✅Going to credit page');
  ...
  async analyze(config) {
    ...
    const hasUsage = await this.checkUsageLimit(cost);
    if (!hasUsage) {
      return null;
    }
    ...
    try {
      ...
      if (useBackground && this.user) {
        ...
        try {
          ...
          const bgParams = { ...params, usedFree, usedCredit, cost };
      ...
      if (saveDataBuilder && !isBackgroundResult) {
        ...
        if (usedFree > 0) {
          saveData.editCount = firestore.increment(usedFree);
        }
        if (usedCredit > 0) {
          saveData.credits = firestore.increment(-usedCredit);
        }
