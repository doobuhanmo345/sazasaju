...
class TarotAnalysisService {
  ...
  checkUsageLimit = async () => {
    ...
    if (this.handleCancelHelper && totalRemaining <= 0) {
  ...
  async analyze(config) {
    ...
    const hasUsage = await this.checkUsageLimit();
    if (!hasUsage) {
      return null;
    }
    ...
    if (currentCount >= this.maxEditCount) {
      if (currentCredits > 0) {
        ...
        // Double check just in case, though checkUsageLimit should catch this
        const msg = this.language === 'ko'
          ? '오늘의 무료 분석 횟수를 모두 소진했습니다.\n크레딧을 충전하여 계속 이용해보세요!'
          : 'Daily free limit reached. Please recharge credits to continue.';
        if (confirm(msg)) {
