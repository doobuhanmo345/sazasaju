...
class TarotAnalysisService {
   ...
   * - onSnapshot으로 결과 대기
   */
  async analyzeInBackground(prompt, userId, targetPath) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        unsubscribe = firestore.onSnapshot(
          ...
          (docSnap) => {
            if (!docSnap.exists()) {
              console.log('[Tarot analyzeInBackground] Document deleted (completed)');
              clearTimeout(timeoutId);
              if (unsubscribe) unsubscribe();
              resolve(null); // Document deleted means completed
              return;
            }
    ...
    try {
      ...
      if (useBackground && this.user) {
        ...
        try {
          ...
          result = await this.analyzeInBackground(fullPrompt, this.user.uid, targetPath);
        } catch (bgError) {
        ...
        if (this.user) {
          try {
            const notifMsg = this.language === 'ko'
              ? `${this.uiText?.analysisComplete?.ko || '타로 분석이 완료되었습니다.'}`
              : 'Tarot analysis completed.';

            await addDoc(collection(db, 'notifications'), {
              userId: this.user.uid,
              type: 'analysis', // or 'tarot'
              message: notifMsg,
              isRead: false,
              createdAt: serverTimestamp(),
              createdAt: serverTimestamp(),
