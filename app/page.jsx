...
// Footer is layout level in Next.js, but legacy had it in App. 
// We generally keep Footer in layout, but if it was conditionally rendered here, we might need to check.
// In RootLayout we already added Footer, so we don't need it here.
...
export default function Home() {
  ...
  const [inputDate, setInputDate] = useState(() => {
    try {
      // Server-side safe time generation for hydration match if possible, 
      // but 'new Date()' locally might differ. 
      // For input default value, it's safer to resolve in useEffect or use fixed string for initial render if critical.
      // However, App Router handles 'use client' hydration.
      // We'll keep it simple for now and rely on useEffect sync.
      const now = new Date();
